PlotThree <- function() {
    ...
    ## set device to PNG
    png(file = "./Exploratory_Data_Analysis/Project_One/Plot3.png")
    
    ## Create plot, then add lines and legend.
    with(result1, plot(result1$DateTime, result1$Sub_metering_1, type = "n", xlab = "", ylab = "Energy Sub metering"))
    with(result1, lines(result1$DateTime, result1$Sub_metering_1, col = "black"))
    with(result1, lines(result1$DateTime, result1$Sub_metering_2, col = "red"))
    with(result1, lines(result1$DateTime, result1$Sub_metering_3, col = "blue"))
